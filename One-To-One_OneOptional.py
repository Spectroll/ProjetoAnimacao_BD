...
passp_att_code = Tex("ID", color= BLACK, font_size= 20).next_to(passp_att, DOWN, buff= 0.05)
...
#Modelo logico de pessoa no metodo Adicao de Atributos
#---------------------------------------------------------------------------------------------------------------------------#
person_att_adding = Table(
    ...
    ["Alex","06"]],
    row_labels=[Text("01"), Text("02"), Text("03"), Text("04"), Text("05"), Text("06")],
    col_labels=[Text("Name"), Text("Passp_ID")],
    top_left_entry=Text("ID"),
    include_outer_lines=True,
    line_config= {"color": BLACK},
    v_buff=0.5, h_buff=0.6).move_to([0, -2, 0])
...
class Introduction(Scene):
    def construct(self):
        ...
        intro = Tex("A one-to-one optional relationship establishes that one instance from each participant entity may or may not have a connection between them.", color= BLACK, font_size= 32).next_to(relation, UP, buff=3.5)
        intro_2 = Tex("In other words, only one side of the relationship will be optional in this relationship type, while the other is mandatory.", color= BLACK, font_size= 32).next_to(intro, DOWN, buff= 0.25)        
        ...
        #Parte 3: Apresentacao dos metodos
...
class Entity_Fusion(Scene):
    def construct(self):
        ...
        intro = Tex("This method implies merging both entities and the relationship into a single relation.", color= BLACK, font_size= 32).next_to(relation, UP, buff= 1)
        ...
        pro_1 = Tex("-Eliminates the need for relations joints", color= BLACK, font_size= 32).next_to(pros, DOWN, buff= 0.25)
        ...
        con_1 = Tex(r"-Inhibits model evolution to attend \\ to new business needs", color= BLACK, font_size= 32).next_to(cons, DOWN, buff= 0.25)
        con_2 = Tex(r"-Optional attributes may only apply \\ to some instances, resulting in many NULL values.", color= BLACK, font_size= 32).next_to(con_1, DOWN, buff= 0.25)
...
class Attribute_Adding(Scene):
    def construct(self):
        ...
        desc = Tex("This method requires adding the attribute key from the mandatory relation to the optional relation.", color= BLACK, font_size= 32).next_to(relation, UP, buff=1)
        ...
        path = ArcBetweenPoints(start= person.get_center(), end= [-2, -1.5, 0], angle= 0.5)
        ...
        person_att_adding.move_to(person_copy.get_center()).shift(DOWN*0.5)
        self.wait()
        self.play(FadeTransform(person_copy, person_att_adding))
        self.wait(2)
        ...
        path = ArcBetweenPoints(start= passp_code.get_center(), end= [-1.1, 0, 0], angle= -1.7)
        ...
        exp = Tex("The relationship will be represented as a Foreign Key between Employee's and Department's relation:", color= BLACK, font_size= 32).next_to(relation, DOWN)
        manage_copy = manage.copy()
        manage_copy.generate_target()
        manage_copy.target.next_to(manage, DOWN, 1.75)
        
        line = Line(start= [-2, 0, 0], end= [0.75, 0, 0]).set_color(PURE_RED)
        line2= Arrow(start= [0.85, -0.1, 0], end= [0.85, -0.47, 0], max_stroke_width_to_length_ratio= 10, max_tip_length_to_length_ratio= 0.5).set_color(PURE_RED)
        line3= Arrow(start= [-2.1, -0.1, 0], end= [-2.1, -0.47, 0], max_stroke_width_to_length_ratio= 10, max_tip_length_to_length_ratio= 0.5).set_color(PURE_RED)
        arco = ArcBetweenPoints(start= line.get_end(), end= line2.get_start(), angle = - PI/2).set_color(PURE_RED)
        arco2 = ArcBetweenPoints(start= line.get_start(), end= line3.get_start()).set_color(PURE_RED)
        fk = VGroup(line, arco, arco2, line2, line3)
        table_group = VGroup(logic_passp, logic_person)
        fk.move_to(table_group.get_top()).shift(UP*0.25).shift(RIGHT*1.1)
        ...
        #Parte 5: Conclusao
        final = VGroup(fk, key, person_att_adding, logic_passp, column_copy)
        pros = Tex("Pros:", color= PURE_GREEN, font_size= 40).move_to([-3, 3, 0])
...
class New_Relation(Scene):
    def construct(self):
        ...
        desc = Tex("For this method, the entities will have their separate relation.", color= BLACK, font_size= 32).next_to(relation, UP, buff=0.5)
        ...
        to_elim = VGroup(desc)
        desc = Tex("Moreover, the relationship will also have its own relation.", color= BLACK, font_size= 32).next_to(relation, UP, buff=0.5)
        ...
        to_elim = VGroup(desc)
        desc = Tex("Note that the relationship's relation is comprised by the primary keys of the entities:", color= BLACK, font_size= 32).next_to(relation, UP, buff=0.5)
        ...
        desc = Tex("Next, the primary key of each entity will be a foreign key to the relationship's relation.", color= BLACK, font_size= 32).next_to(relation, UP, buff=0.5)
        ...
        pros = Tex("Pros:", color= PURE_GREEN, font_size= 40).move_to([-3, 3, 0])
        pro_1 = Tex("-Model is VERY extendable", color= BLACK, font_size= 32).next_to(pros, DOWN, buff= 0.25)
        cons = Tex("Cons:", color= PURE_RED, font_size= 40).move_to([3, 3, 0])
        con_1 = Tex("-More complex relations' association.", color= BLACK, font_size= 40).next_to(cons, DOWN, buff= 0.25)
